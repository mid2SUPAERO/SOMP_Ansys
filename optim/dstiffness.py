...
def dk2d(Ex,Ey,nuxy,nuyz,Gxy,T,V):
    ...
    dkdt = np.zeros((len(T),8,8))
    for xi, wi in zip(points, weights):
        for xj, wj in zip(points, weights):
                B = B2d(xi, xj)
                dkdt += wi * wj * B.T @ (dTt @ C @ Tt.transpose((0,2,1)) + Tt @ C @ dTt.transpose((0,2,1))) @ B
    
    dkdt *= V[:,np.newaxis,np.newaxis]
    return dkdt, np.zeros((len(T),8,8))
...
def dk3d(Ex,Ey,nuxy,nuyz,Gxy,T,A,V,print_euler):
    ...
    Tprint = Ta_fun(np.array([euler2])) @ Tt_fun(np.array([euler1]))
    C = Tprint @ C @ Tprint.transpose((0,2,1))
    
    dkdt, dkda = np.zeros((len(T),24,24)), np.zeros((len(A),24,24))
    for xi, wi in zip(points, weights):
        for xj, wj in zip(points, weights):
            for xk, wk in zip(points, weights):
                B = B3d(xi, xj, xk)
                dkdt += wi * wj * wk * B.T @ Ta @ (dTt @ C @ Tt.transpose((0,2,1)) + Tt @ C @ dTt.transpose((0,2,1))) @ Ta.transpose((0,2,1)) @ B
                dkda += wi * wj * wk * B.T @ (dTa @ Tt @ C @ Tt.transpose((0,2,1)) @ Ta.transpose((0,2,1)) + Ta @ Tt @ C @Tt.transpose((0,2,1)) @ dTa.transpose((0,2,1))) @ B
    
    dkdt *= V[:,np.newaxis,np.newaxis]
    dkda *= V[:,np.newaxis,np.newaxis]
    return dkdt, dkda
