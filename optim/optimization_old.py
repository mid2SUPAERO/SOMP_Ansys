...
class TopOpt():
    ...
    def __init__(self, inputfiles, Ex, Ey, nuxy, nuyz, Gxy, volfrac, r_rho=0, r_theta=0, print_direction=(0.,0.,1.), overhang_angle=45, overhang_constraint=False, initial_angles_type='fix', theta0=0, alpha0=0, max_iter=200, tol=0, continuation=True, dim='3D_layer', jobname=None, echo=True):
        ...
        self.overhang_local, self.overhang_support, self.overhang_boundary, self.r_s, self.T, self.betaT = self.overhang_neighborhoods()
    ...
    def sensitivities(self, x):
        ...
        if self.overhang_constraint: # design variable in x is psi instead of rho
            psi = rho.copy()
            phi, rho_s, mu_s = self.printability(psi)[0:3]
            rho = 1 - np.exp(-self.beta*phi) + phi*np.exp(-self.beta)
            rho[rho < self.rho_min] = self.rho_min
...
#             if it >= 10 and np.abs((np.sum(self.comp_max_hist[-10:])-np.sum(self.comp_max_hist[-11:-1]))/np.sum(self.comp_max_hist[-11:-1])) < self.tol:
            if it >= 1 and np.abs(self.comp_max_hist[-1]-self.comp_max_hist[-2])/self.comp_max_hist[-2] < self.tol:
                ...
                if self.get_greyness(self.rho_hist[-1]) < self.grey_thr:
                    break
                else:
        ...
        _, _, _, self.elm_printability, self.print_score = self.printability(self.rho_hist[-1])
        if self.echo: print('\nPrintability score = {:.3f}'.format(self.print_score))
        
        self.greyness = self.get_greyness(rho)
        if self.echo: print('Greyness = {:.3f}'.format(self.greyness))
    ...
    def overhang_neighborhoods(self):
        h     = np.mean(np.cbrt(self.elemvol)) # finite element size
        r_s   = self.r_rho # radius of the neighborhood set
        T     = 1/(np.pi/2-self.overhang_angle) * h/(2*r_s) # Heaviside threshold
        betaT = 25 # Heaviside parameter
        ...
        return local, support, boundary, r_s, T, betaT
    
    def printability(self, psi):
        phi  = np.copy(psi)
        beta = self.beta
        T    = self.T
